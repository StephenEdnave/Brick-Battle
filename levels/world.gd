...
func new_level(index):
	if current_level == levels.size():
		current_level = 0
		index = 0

	brick_max = brick_count
	var _new_level = load(levels[index]).instance()
	_new_level.connect("level_win", self, "level_win")
	_new_level.connect("brick_spawned", self, "brick_spawned")
	_new_level.connect("brick_death", self, "brick_died")
	_new_level.spawn_bricks()
	add_child(_new_level)

	$Background/Background.modulate.r = (sin((index * 30) * (PI / 180) + 0) * 30 + 100) / 255
	$Background/Background.modulate.g = (sin((index * 30) * (PI / 180) + 2) * 30 + 80) / 255
	$Background/Background.modulate.b = (sin((index * 30) * (PI / 180) + 4) * 30 + 128) / 255
...
func _on_StartCountdownTimer_timeout():
	...
	if start_countdown <= 0 and balls < num_players:
